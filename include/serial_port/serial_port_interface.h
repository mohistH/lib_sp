...
//---------------------------------------------------------------------------------------------
	...
	*/
	struct serial_port_info_
	{
		...
		serial_port_info_() noexcept
		{
			zero();
		}
	};
	// to output serial port information
	using sp_info = serial_port_info_;
	...
	*/
	...
	using serial_port_prop = serial_port_prop_;
	...
	*/
	...
	public:
		...
		/**
			...
			*/
		virtual int init(const serial_port_prop& spp, irecv_data* precv_data = nullptr) = 0;
	...
	/*
	* @brief: the utils of serial port
	*/
	class _lib_sp_api_ serial_port_info
	{
	public:
		/** 
		*  @ brief: get information of serial port
		*  @ return - std::list<sp_info>
...
//--------------------------------------------------------------------------------------
	...
	/*
	*  @ brief: to use iserial_port on windows, it will create an iserial_port object
				it returns NULL while build object
	*  @ return - iserial_port *
				NULL - failure
				XXX - success
	*/
	extern "C" _lib_sp_api_ iserial_port *sp_create_win() noexcept;
